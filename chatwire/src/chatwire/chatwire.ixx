...
import std;
// Re-exported, not merely imported: `chatwire::version` and
// `chatwire::default_port` are part of this module's surface and always were.
// They live in a leaf module so that a feature can read them without importing
// the root -- see the top of chatwire/version.ixx for the cycle that forced it.
export import chatwire.version;
...
//
// ===========================================================================
// WHY THE BODIES ARE IN THIS FILE
// ===========================================================================
// start() and stop() used to live in a src/chatwire.cpp implementation unit,
// because as a HEADER this file could not afford them: the bodies reach vmhook's
// 27,000 lines and Winsock's extern "C" block, and every consumer would have
// compiled both.
//
// A module has no such cost.  `import chatwire.api;` reads a compiled BMI --
// what this unit imports is this unit's business, and a consumer pays for the
// declarations it can see, not for the graph behind them.  Nor is there any
// include ordering left to police: winsock2.h-before-windows.h is a rule about
// text, and there is no text crossing this boundary.
//
// One thing DOES still matter, and it is the reason the definitions below are
// not marked `inline`: GCC 16.2 emits an inline function's local static into
// every consumer's object as well as this one's.  See the note on
// chatwire::registry::detail::storage() in chatwire.feature.


// The public surface: a host imports this one module and gets start/stop,
// logging, the feature interface, the JSON helpers and the mapping tables.
//
// Not exported: chatwire.sdk and chatwire.ws.server.  They are implementation
// detail -- a host talks to chatwire through start()/stop() and the feature
// interface, never through vmhook or a socket -- and an import is not
// transitive unless it says `export import`, so importing this module does not
// put either of them in anybody's namespace.
export namespace chatwire
{
    /*
        ...
        Declared here and defined further down the same unit, non-inline: the
        body runs once and has no business being emitted into every consumer.
...
// ===========================================================================
// The bodies of start() / stop() and the wiring they use.
// ===========================================================================
// Everything from here down is the module purview, not the exported surface:
// chatwire::detail is not in the `export namespace` block above, so a host that
// imports chatwire.api cannot name any of it.  That is the same boundary the
// separate .cpp drew, enforced by the language instead of by which file the
// text happened to sit in.
//
// The startup ORDER is not arbitrary -- see the top of this file.
...
namespace chatwire
{
    ...
    */
        ...
        -> bool
    {
        ...
        //     rather than self-registering — see chatwire.features.chat for why.
